...
if(isset($_GET['msg']) && $_GET['msg']=='editexam'){
	$examid=$_GET['id'];
	$arr_examdtls=$exm->getAllExamDetails($examid);
	
	$examid=$arr_examdtls[0];
	$examname=$arr_examdtls[1];
	$rules=$arr_examdtls[2];
	$enddate=$arr_examdtls[3];
	$time=$arr_examdtls[4];
	$noofques=$arr_examdtls[5];
	$maxmarks=$arr_examdtls[6];
	$minmarks=$arr_examdtls[7];
	$mca=$arr_examdtls[8];
	$mwa=$arr_examdtls[9];
	$subjectname=$arr_examdtls[10];
	$testname=$arr_examdtls[11];
	$subjectid=$arr_examdtls[12];
	$testid=$arr_examdtls[13];
					...
					<?php
						...
						if($arr_sub){
							foreach($arr_sub as $val_sub){
								//print_r($val_sub);
								echo '<option value="'.$val_sub['subject_id'].'"';
								if($val_sub['subject_id']==$subjectid) echo 'selected="selected"';
								echo '/>'.$val_sub['subject_name'].' ('.$val_sub['course_name'].' '.$val_sub['semister_name'].')</option>';
							}
						}
					?>	
				</select>
				...
				<select name="skilltest" id="skilltest">
					<?php 
						...
						foreach($arr_test as $val_test){
							echo '<option value="'.$val_test['test_id'];
							if($val_test['test_id']==$testid) echo 'selected="selected"';
							echo '">'.$val_test['test_name'].'</option>';
						}
					?>
				</select>
			</td>
